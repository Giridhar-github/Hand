...
la_label =  np.ones(len(la_imgs))*1
...
na_label =  np.ones(len(na_imgs))*2

imgs.extend(na_imgs)

path = "data/pa/"
pa_imgs = loadImages(path)
pa_label = np.ones(len(pa_imgs))*3

imgs.extend(pa_imgs)

path = "data/ra/"
ra_imgs = loadImages(path)
ra_label = np.ones(len(ra_imgs))*4

imgs.extend(ra_imgs)

path = "data/rr/"
rr_imgs = loadImages(path)
rr_label = np.ones(len(rr_imgs))*5

imgs.extend(rr_imgs)

path = "data/sa/"
sa_imgs = loadImages(path)
sa_label = np.ones(len(sa_imgs))*6
imgs.extend(sa_imgs)

path = "data/tha/"
tha_imgs = loadImages(path)
tha_label = np.ones(len(tha_imgs))*7
...
labels = np.hstack((aa_label,la_label,na_label,pa_label,
                    ra_label,rr_label,sa_label,
					tha_label))#,ssa_label,tha_label))
